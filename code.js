...
document.getElementById("game-container").style="padding-left:10%; padding-top:11%; -moz-transform: scale(1.3);";
...
//MOVABLE
var x=0,y=0,m=10;
$(document).keydown(function(event){//0numpad, arrowKeys(left, right, up, down)
	if (event.which==96) {if (m==10) m=2; else m=10;}
	else if (event.which==37) x-=m;
	else if (event.which==39) x+=m;
	else if (event.which==38) y-=m;
	else if (event.which==40) y+=m;
	for (i=0;i<6;i++) {document.getElementById("iframe-game").contentWindow.document.getElementsByClassName("imgClass")[i].style.left = x+"px";}
	for (i=0;i<6;i++) {document.getElementById("iframe-game").contentWindow.document.getElementsByClassName("imgClass")[i].style.top = y+"px";}
	...
	else {rotFct(); document.getElementById("iframe-game").contentWindow.document.getElementsByClassName("imgClass")[0].src="https://i.imgur.com/hwpPFom.png"; for (i=1;i<6;i++) document.getElementById("iframe-game").contentWindow.document.getElementsByClassName("imgClass")[i].style.visibility="visible";}	
});
